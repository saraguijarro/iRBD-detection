...
# PREPROCESSING VV - VALIDATED VERSION (BRINK'S METHODOLOGY)
# Transform raw accelerometer files into clean night-segmented data suitable for feature extraction.
# VV: Per-night temperature filtering (27°C) + TST validation, following Brink-Kjær et al. (2023)
...
## VERSION COMPARISON:
# - v0: Baseline - actipy non-wear only, fixed 22:00-06:00 windows, NO temperature
# - v1: Fixed 22:00-06:00 windows + per-sample temperature (18°C, DETACH algorithm)
# - vv (this): Fixed 22:00-06:00 windows + per-night temperature (27°C, Brink's approach)
# Purpose: Implement Brink's validated methodology for comparison
...
#    - Apply per-night temperature filtering (avg temp >= 27°C)
#    - Apply TST validation (3h <= TST <= 12h)
#    - Apply nonwear validation (< 2h per night)
...
# Directories : 
#    - /work3/s184484/iRBD-detection/data/preprocessed_vv/controls/
#    - /work3/s184484/iRBD-detection/data/preprocessed_vv/irbd/ 
...
# - Baseline data quality assessment (no temperature filtering applied)
...
# Temperature Threshold : 27°C per-night average (Brink's validated approach)
# TST Validation : 3-12 hours (Brink's heuristic exclusion rules)
# Non-wear Detection : Automatic flagging by actipy + < 2h nonwear per night
...
## HPC JOB EXECUTION
# Job script : preprocessing_vv_job.sh
# Location : /work3/s184484/iRBD-detection/jobs/scripts/preprocessing_vv_job.sh
# Queue : hpc (CPU nodes)
# Resources : 4 cores, 8GB RAM per core
# Time limit : 24 hours
# Output logs : /work3/s184484/iRBD-detection/jobs/logs/prep_vv/prep_vv_output_JOBID.out
# Error logs : /work3/s184484/iRBD-detection/jobs/logs/prep_vv/prep_vv_error_JOBID.err
...
plt.style.use('seaborn-v0_8')     # Use seaborn's visual style
...
class PreprocessingPipeline:
    ...
    4. Removes poor quality data (actipy non-wear detection)
    5. Saves clean data in HDF5 format for the next pipeline stage

    VERSION VV (VALIDATED - BRINK'S METHODOLOGY):
    - Per-night temperature filtering (27°C average threshold)
    - TST validation (3-12 hours per night)
    - Nonwear validation (< 2 hours per night)
    - Fixed 22:00-06:00 time windows (8-hour sleep periods)
    - Purpose: Implement Brink-Kjær et al. (2023) validated methodology
    - Saves to preprocessed_vv/ directory
    ...
    - 27°C threshold: Brink's validated approach for non-wear detection
    - 3-12h TST: Brink's heuristic exclusion rules for data quality
    """
    ...
    def __init__(self):
        ...
        self.preprocessed_controls_dir = self.base_dir / "data" / "preprocessed_vv" / "controls"  # Processed healthy data
        self.preprocessed_irbd_dir = self.base_dir / "data" / "preprocessed_vv" / "irbd"          # Processed iRBD data

        # Visualization output directory (where to save plots for the report):
        self.plots_dir = self.base_dir / "results" / "visualizations" / "vv"
        ...
        # VV: Per-night temperature threshold (Brink's validated approach)
        self.temp_threshold = 27.0              # Temperature threshold in Celsius (per-night average)
        self.min_tst_hours = 3.0                # Minimum total sleep time per night
        self.max_tst_hours = 12.0               # Maximum total sleep time per night
        self.max_nonwear_hours = 2.0            # Maximum nonwear hours per night
        
        # WHY THESE THRESHOLDS (BRINK'S METHODOLOGY):
        # - 27°C: Brink-Kjær et al. (2023) validated threshold for non-wear detection
        # - 3-12h TST: Heuristic exclusion rules to remove nights with insufficient/excessive data
        # - < 2h nonwear: Ensures sufficient valid data for analysis
        ...
        print(f"Temperature threshold: {self.temp_threshold}°C (per-night average, Brink's approach)")
        print(f"TST range: {self.min_tst_hours}-{self.max_tst_hours} hours")
        print(f"Max nonwear: {self.max_nonwear_hours} hours per night")
    ...
    def initialize_stats(self):
        ...
        self.stats = {
            ...
            'nights_rejected_tst_short': 0, # Nights rejected: TST < 3h
            'nights_rejected_tst_long': 0,  # Nights rejected: TST > 12h
            'nights_rejected_temperature': 0, # Nights rejected: avg temp < 27°C
            'nights_rejected_nonwear': 0,   # Nights rejected: nonwear > 2h
            ...
            'nonwear_filtered_hours': 0.0,  # Hours removed by nonwear filtering
            'avg_temperature_valid': [],    # List of avg temps for valid nights
            'avg_temperature_rejected': [], # List of avg temps for rejected nights
            'tst_valid': [],                # List of TST for valid nights
            'tst_rejected': []              # List of TST for rejected nights
        }
    ...
    def setup_logging(self):
        ...
        log_file = self.log_dir / f"preprocessing_vv_{current_time}.log"
        ...
        self.logger.info(f"=== Preprocessing Pipeline VV Started ({self.mode}) ===")
        self.logger.info(f"Base directory: {self.base_dir}")
        self.logger.info(f"Temperature threshold: {self.temp_threshold}°C (per-night average, Brink's approach)")
        self.logger.info(f"TST range: {self.min_tst_hours}-{self.max_tst_hours} hours")
        self.logger.info(f"Max nonwear: {self.max_nonwear_hours} hours per night")
    ...
    def apply_nonwear_filtering(self, data):
        """
        v0: Remove only actipy-flagged non-wear periods (NO temperature filtering).
        
        We only remove periods that actipy flagged as non-wear (NaN values).

        Args:
            data: DataFrame containing accelerometer data

        Returns:
            DataFrame: Filtered data with only actipy-validated wear periods
        """
        ...
        # Apply only the actipy non-wear filter (NO temperature filtering)
        filtered_data = data[valid_data_mask].copy()
        ...
        # Update our statistics (using nonwear instead of temperature)
        self.stats['nonwear_filtered_hours'] += removed_hours

        # Check if we have any data left after filtering
        if len(filtered_data) == 0:
            raise ValueError("No data remaining after non-wear filtering")
    ...
    def calculate_night_average_temperature(self, night_data):
        """
        Calculate average temperature for a single night.
        
        Args:
            night_data: DataFrame with temperature column
            
        Returns:
            float: Average temperature in Celsius
        """
        # Remove NaN values (actipy nonwear)
        valid_temps = night_data['temperature'].dropna()
        
        if len(valid_temps) == 0:
            return 0.0  # No valid data
        
        return valid_temps.mean()

    def calculate_total_sleep_time(self, night_data):
        """
        Calculate total sleep time (valid data hours) for a night.
        
        Args:
            night_data: DataFrame with accelerometer data
            
        Returns:
            float: TST in hours
        """
        # Count non-NaN samples
        valid_samples = night_data['x'].notna().sum()
        
        # Convert to hours
        tst_hours = valid_samples / self.resample_hz / 3600
        
        return tst_hours

    def calculate_nonwear_hours(self, night_data):
        """
        Calculate nonwear hours for a night.
        ...
        Returns:
            float: Nonwear hours
        """
        # Count NaN samples (actipy marks nonwear as NaN)
        nonwear_samples = night_data['x'].isna().sum()
        
        # Convert to hours
        nonwear_hours = nonwear_samples / self.resample_hz / 3600
        
        return nonwear_hours

    def validate_night(self, night_data):
        """
        Apply Brink's three exclusion rules to a single night.
        
        BRINK'S EXCLUSION RULES (from 2023 paper):
        1. TST must be between 3 and 12 hours
        2. Average temperature must be >= 27°C
        3. Nonwear must be < 2 hours
        
        Args:
            night_data: DataFrame with accelerometer and temperature data
            
        Returns:
            tuple: (is_valid, reason, metrics) where:
                - is_valid: Boolean indicating if night passes all rules
                - reason: String explaining why night was rejected (or "Valid")
                - metrics: Dict with calculated metrics (tst, avg_temp, nonwear)
        """
        # Calculate metrics
        tst = self.calculate_total_sleep_time(night_data)
        avg_temp = self.calculate_night_average_temperature(night_data)
        nonwear = self.calculate_nonwear_hours(night_data)
        ...
        # Rule 1: TST must be between 3 and 12 hours
        if tst < self.min_tst_hours:
            return False, f"TST too short ({tst:.1f}h < {self.min_tst_hours}h)", metrics
        if tst > self.max_tst_hours:
            return False, f"TST too long ({tst:.1f}h > {self.max_tst_hours}h)", metrics
        
        # Rule 2: Average temperature must be >= 27°C
        if avg_temp < self.temp_threshold:
            return False, f"Avg temp too low ({avg_temp:.1f}°C < {self.temp_threshold}°C)", metrics
        
        # Rule 3: Nonwear must be < 2 hours
        if nonwear > self.max_nonwear_hours:
            return False, f"Too much nonwear ({nonwear:.1f}h > {self.max_nonwear_hours}h)", metrics
        
        return True, "Valid", metrics

    def filter_nights_by_quality(self, nights_data):
        """
        Filter nights using Brink's three exclusion rules.
        
        This applies per-night validation and removes nights that don't meet
        ...
        Returns:
            List of dictionaries containing only valid nights
        """
        self.logger.info(f"Applying Brink's exclusion rules to {len(nights_data)} nights")
        self.logger.info(f"   - TST range: {self.min_tst_hours}-{self.max_tst_hours} hours")
        self.logger.info(f"   - Temperature threshold: >= {self.temp_threshold}°C")
        self.logger.info(f"   - Nonwear threshold: < {self.max_nonwear_hours} hours")
        
        valid_nights = []
        
        for night in nights_data:
            night_data = night['data']
            is_valid, reason, metrics = self.validate_night(night_data)
            
            if is_valid:
                valid_nights.append(night)
                self.stats['valid_nights'] += 1
                self.stats['avg_temperature_valid'].append(metrics['avg_temp'])
                self.stats['tst_valid'].append(metrics['tst'])
                self.logger.info(f"   - Night {night['night_number']}: VALID "
                               f"(TST={metrics['tst']:.1f}h, Temp={metrics['avg_temp']:.1f}°C, "
                               f"Nonwear={metrics['nonwear']:.1f}h)")
            else:
                self.stats['avg_temperature_rejected'].append(metrics['avg_temp'])
                self.stats['tst_rejected'].append(metrics['tst'])
                
                # Update rejection statistics
                if "TST too short" in reason:
                    self.stats['nights_rejected_tst_short'] += 1
                elif "TST too long" in reason:
                    self.stats['nights_rejected_tst_long'] += 1
                elif "Avg temp too low" in reason:
                    self.stats['nights_rejected_temperature'] += 1
                elif "Too much nonwear" in reason:
                    self.stats['nights_rejected_nonwear'] += 1
                
                self.logger.info(f"   - Night {night['night_number']}: REJECTED - {reason}")
        
        self.logger.info(f"   - Valid nights: {len(valid_nights)}/{len(nights_data)} "
                       f"({100*len(valid_nights)/len(nights_data):.1f}%)")
    ...
    def process_participant(self, cwa_file, group_type):
        ...
        try:
            ...
            # STEP 2: Apply non-wear removal
            filtered_data = self.apply_nonwear_filtering(accel_data)

            # STEP 3: Segment data into individual nights
            nights_data = self.segment_nights(filtered_data)
            
            # Update total nights count
            self.stats['total_nights'] += len(nights_data)

            # STEP 4: Apply Brink's per-night quality filtering
            valid_nights = self.filter_nights_by_quality(nights_data)
            
            # Check if we have any valid nights left
            if len(valid_nights) == 0:
                self.logger.warning(f"No valid nights remaining for {participant_id} after quality filtering")
                self.stats['failed_files'] += 1
                return

            # STEP 5: Save preprocessed data to HDF5 file
            ...
            self.save_preprocessed_data(valid_nights, participant_id, output_path)

            # STEP 6: Verify the saved file structure
            ...
            total_hours = sum(night['duration_hours'] for night in valid_nights)
    ...
    def print_final_statistics(self):
        ...
        self.logger.info(f"Data Quality (Brink's Exclusion Rules):")
        self.logger.info(f"  - Total nights: {self.stats['total_nights']}")
        self.logger.info(f"  - Valid nights: {self.stats['valid_nights']}")
        
        # Calculate rejection statistics
        total_rejected = (self.stats['nights_rejected_tst_short'] + 
                         self.stats['nights_rejected_tst_long'] + 
                         self.stats['nights_rejected_temperature'] + 
                         self.stats['nights_rejected_nonwear'])
        
        if self.stats['total_nights'] > 0:
            retention_rate = self.stats['valid_nights'] / self.stats['total_nights'] * 100
            self.logger.info(f"  - Retention rate: {retention_rate:.1f}%")
        
        self.logger.info(f"\n  Rejection Breakdown:")
        self.logger.info(f"    - TST < {self.min_tst_hours}h: {self.stats['nights_rejected_tst_short']}")
        self.logger.info(f"    - TST > {self.max_tst_hours}h: {self.stats['nights_rejected_tst_long']}")
        self.logger.info(f"    - Avg temp < {self.temp_threshold}°C: {self.stats['nights_rejected_temperature']}")
        self.logger.info(f"    - Nonwear > {self.max_nonwear_hours}h: {self.stats['nights_rejected_nonwear']}")
        self.logger.info(f"    - Total rejected: {total_rejected}")
        
        self.logger.info(f"\n  - Valid data hours: {self.stats['valid_hours']:.1f}")
        
        # Temperature statistics
        if len(self.stats['avg_temperature_valid']) > 0:
            avg_temp_valid = np.mean(self.stats['avg_temperature_valid'])
            self.logger.info(f"  - Avg temperature (valid nights): {avg_temp_valid:.1f}°C")
        if len(self.stats['avg_temperature_rejected']) > 0:
            avg_temp_rejected = np.mean(self.stats['avg_temperature_rejected'])
            self.logger.info(f"  - Avg temperature (rejected nights): {avg_temp_rejected:.1f}°C")
