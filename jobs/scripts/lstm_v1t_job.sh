...
#BSUB -R "span[hosts=1]"
#BSUB -W 24:00
#BSUB -o /work3/s184484/iRBD-detection/jobs/logs/lstm/lstm_v1t_%J.out
#BSUB -e /work3/s184484/iRBD-detection/jobs/logs/lstm/lstm_v1t_%J.err
#BSUB -N

# LSTM TRAINING JOB SCRIPT FOR iRBD DETECTION PIPELINE - VERSION V1t
...
# V1t: Fixed nights + temperature filter
...
echo "Started: $(date)"
echo ""
echo "Preprocessing version: v1t (Fixed nights + temp filter)"
echo "=========================================="
...
# Feature directory for V1t
FEATURES_DIR="data/features_v1t"
...
if [ ! -d "$FEATURES_DIR/controls" ]; then
    ...
    echo "Make sure feature extraction stage completed successfully for v1t"
    exit 1
fi
...
mkdir -p results/lstm_v1t/models
mkdir -p results/lstm_v1t/predictions
mkdir -p results/lstm_v1t/evaluation
mkdir -p results/lstm_v1t/interpretability
mkdir -p results/visualizations/v1t
...
echo "Executing: python scripts/lstm.py --version v1t"
python scripts/lstm.py --version v1t
...
# Check for any output files in results/lstm_v1t
TOTAL_OUTPUT_FILES=0

# Check models
if [ -d "results/lstm_v1t/models" ]; then
    MODEL_COUNT=$(find results/lstm_v1t/models -type f 2>/dev/null | wc -l)
    echo "Model files: $MODEL_COUNT"
    TOTAL_OUTPUT_FILES=$((TOTAL_OUTPUT_FILES + MODEL_COUNT))
fi

# Check predictions
if [ -d "results/lstm_v1t/predictions" ]; then
    PRED_COUNT=$(find results/lstm_v1t/predictions -type f 2>/dev/null | wc -l)
    echo "Prediction files: $PRED_COUNT"
    TOTAL_OUTPUT_FILES=$((TOTAL_OUTPUT_FILES + PRED_COUNT))
fi

# Check evaluation
if [ -d "results/lstm_v1t/evaluation" ]; then
    EVAL_COUNT=$(find results/lstm_v1t/evaluation -type f 2>/dev/null | wc -l)
    echo "Evaluation files: $EVAL_COUNT"
    TOTAL_OUTPUT_FILES=$((TOTAL_OUTPUT_FILES + EVAL_COUNT))
fi

# Check interpretability
if [ -d "results/lstm_v1t/interpretability" ]; then
    INTERP_COUNT=$(find results/lstm_v1t/interpretability -type f 2>/dev/null | wc -l)
...
echo "JOB SUMMARY"
echo "=========================================="
echo "Version: v1t (Fixed nights + temp filter)"
