...
#BSUB -R "span[hosts=1]"
#BSUB -W 24:00
#BSUB -o /work3/s184484/iRBD-detection/jobs/logs/lstm/lstm_v0_%J.out
#BSUB -e /work3/s184484/iRBD-detection/jobs/logs/lstm/lstm_v0_%J.err
#BSUB -N

# LSTM TRAINING JOB SCRIPT FOR iRBD DETECTION PIPELINE - VERSION V0
...
# V0: Fixed nights WITHOUT temperature filter
...
echo "Started: $(date)"
echo ""
echo "Preprocessing version: v0 (Fixed nights WITHOUT temp filter)"
echo "=========================================="
...
# Feature directory for V0
FEATURES_DIR="data/features_v0"
...
if [ ! -d "$FEATURES_DIR/controls" ]; then
    ...
    echo "Make sure feature extraction stage completed successfully for v0"
    exit 1
fi
...
mkdir -p results/lstm_v0/models
mkdir -p results/lstm_v0/predictions
mkdir -p results/lstm_v0/evaluation
mkdir -p results/lstm_v0/interpretability
mkdir -p results/visualizations/v0
...
echo "Executing: python scripts/lstm.py --version v0"
python scripts/lstm.py --version v0
...
# Check for any output files in results/lstm_v0
TOTAL_OUTPUT_FILES=0

# Check models
if [ -d "results/lstm_v0/models" ]; then
    MODEL_COUNT=$(find results/lstm_v0/models -type f 2>/dev/null | wc -l)
    echo "Model files: $MODEL_COUNT"
    TOTAL_OUTPUT_FILES=$((TOTAL_OUTPUT_FILES + MODEL_COUNT))
fi

# Check predictions
if [ -d "results/lstm_v0/predictions" ]; then
    PRED_COUNT=$(find results/lstm_v0/predictions -type f 2>/dev/null | wc -l)
    echo "Prediction files: $PRED_COUNT"
    TOTAL_OUTPUT_FILES=$((TOTAL_OUTPUT_FILES + PRED_COUNT))
fi

# Check evaluation
if [ -d "results/lstm_v0/evaluation" ]; then
    EVAL_COUNT=$(find results/lstm_v0/evaluation -type f 2>/dev/null | wc -l)
    echo "Evaluation files: $EVAL_COUNT"
    TOTAL_OUTPUT_FILES=$((TOTAL_OUTPUT_FILES + EVAL_COUNT))
fi

# Check interpretability
if [ -d "results/lstm_v0/interpretability" ]; then
    INTERP_COUNT=$(find results/lstm_v0/interpretability -type f 2>/dev/null | wc -l)
...
echo "JOB SUMMARY"
echo "=========================================="
echo "Version: v0 (Fixed nights WITHOUT temp filter)"
