...
#BSUB -R "span[hosts=1]"
#BSUB -W 24:00
#BSUB -o /work3/s184484/iRBD-detection/jobs/logs/lstm/lstm_vvt_%J.out
#BSUB -e /work3/s184484/iRBD-detection/jobs/logs/lstm/lstm_vvt_%J.err
#BSUB -N

# LSTM TRAINING JOB SCRIPT FOR iRBD DETECTION PIPELINE - VERSION vvt
...
mkdir -p results/lstm_vvt/models
mkdir -p results/lstm_vvt/predictions
mkdir -p results/lstm_vvt/evaluation
mkdir -p results/lstm_vvt/interpretability
...
echo "Executing: python scripts/lstm.py --version vvt"
python scripts/lstm.py --version vvt
...
# Check for any output files in results/lstm_vvt
TOTAL_OUTPUT_FILES=0

# Check models
if [ -d "results/lstm_vvt/models" ]; then
    MODEL_COUNT=$(find results/lstm_vvt/models -type f 2>/dev/null | wc -l)
    echo "Model files: $MODEL_COUNT"
    TOTAL_OUTPUT_FILES=$((TOTAL_OUTPUT_FILES + MODEL_COUNT))
fi

# Check predictions
if [ -d "results/lstm_vvt/predictions" ]; then
    PRED_COUNT=$(find results/lstm_vvt/predictions -type f 2>/dev/null | wc -l)
    echo "Prediction files: $PRED_COUNT"
    TOTAL_OUTPUT_FILES=$((TOTAL_OUTPUT_FILES + PRED_COUNT))
fi

# Check evaluation
if [ -d "results/lstm_vvt/evaluation" ]; then
    EVAL_COUNT=$(find results/lstm_vvt/evaluation -type f 2>/dev/null | wc -l)
    echo "Evaluation files: $EVAL_COUNT"
    TOTAL_OUTPUT_FILES=$((TOTAL_OUTPUT_FILES + EVAL_COUNT))
fi

# Check interpretability
if [ -d "results/lstm_vvt/interpretability" ]; then
    INTERP_COUNT=$(find results/lstm_vvt/interpretability -type f 2>/dev/null | wc -l)
