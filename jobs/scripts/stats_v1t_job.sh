...
#BSUB -R "span[hosts=1]"
#BSUB -W 6:00
#BSUB -o /work3/s184484/iRBD-detection/jobs/logs/stats/stats_v1t_%J.out
#BSUB -e /work3/s184484/iRBD-detection/jobs/logs/stats/stats_v1t_%J.err
#BSUB -N

# STATISTICAL ANALYSIS JOB SCRIPT FOR iRBD DETECTION PIPELINE - VERSION V1t
...
mkdir -p results/statistical_analysis_v1t
...
echo "Executing: python scripts/statistical_analysis.py --version v1t"
python scripts/statistical_analysis.py --version v1t
...
# Check for output files in results/statistical_analysis_v1t
TOTAL_OUTPUT_FILES=0

if [ -d "results/statistical_analysis_v1t" ]; then
    STATS_COUNT=$(find results/statistical_analysis_v1t -type f 2>/dev/null | wc -l)
    echo "Statistical results files: $STATS_COUNT"
    TOTAL_OUTPUT_FILES=$((TOTAL_OUTPUT_FILES + STATS_COUNT))
fi

if [ -d "results/visualizations/v1t" ]; then
    VIZ_COUNT=$(find results/visualizations/v1t -type f 2>/dev/null | wc -l)
