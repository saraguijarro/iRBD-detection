...
#BSUB -R "span[hosts=1]"
#BSUB -W 24:00
#BSUB -o /work3/s184484/iRBD-detection/jobs/logs/prep_vv/prep_vv_output_%J.out
#BSUB -e /work3/s184484/iRBD-detection/jobs/logs/prep_vv/prep_vv_error_%J.err
#BSUB -N

# PREPROCESSING VV JOB SCRIPT FOR iRBD DETECTION PIPELINE
# VERSION VV: PER-NIGHT TEMPERATURE FILTERING (BRINK'S VALIDATED METHODOLOGY)
# Implements Brink-Kjær et al. (2023) approach:
#   - Per-night average temperature >= 27°C
...
# =============================================================================
echo "=========================================="
echo "iRBD Detection Pipeline - Preprocessing VV"
...
# Create VV output directories if they don't exist
echo "Creating VV output directories..."
mkdir -p data/preprocessed_vv/controls
mkdir -p data/preprocessed_vv/irbd
mkdir -p jobs/logs/prep_vv
mkdir -p results/visualizations/vv

echo "VV directories created:"
echo "  - data/preprocessed_vv/controls"
echo "  - data/preprocessed_vv/irbd"
echo "  - results/visualizations/vv"
...
# =============================================================================
# PREPROCESSING VV EXECUTION
# =============================================================================
echo "=========================================="
echo "Starting preprocessing VV pipeline"
echo "Methodology: Brink's validated approach"
echo "Temperature threshold: 27°C (per-night average)"
...
# Run preprocessing VV script
echo "Executing: python scripts/preprocessing_vv.py"
python scripts/preprocessing_vv.py
...
echo "Preprocessing VV completed with exit code: $PREPROCESSING_EXIT_CODE"
echo "Total processing time: $DURATION seconds ($(($DURATION / 60)) minutes)"

# =============================================================================
# OUTPUT VERIFICATION
# =============================================================================
echo "Verifying preprocessing VV outputs..."

# Count output files
CONTROLS_OUTPUT=$(find data/preprocessed_vv/controls -name "*.h5" 2>/dev/null | wc -l)
IRBD_OUTPUT=$(find data/preprocessed_vv/irbd -name "*.h5" 2>/dev/null | wc -l)
...
# Check for log files
if [ -f "validation/data_quality_reports/preprocessing_vv_"*".log" ]; then
    echo "✓ Preprocessing log file created"
    LOG_FILE=$(ls -t validation/data_quality_reports/preprocessing_vv_*.log | head -1)
...
if [ $TOTAL_OUTPUT -gt 0 ]; then
    ...
    SAMPLE_H5=$(find data/preprocessed_vv -name "*.h5" | head -1)
    if [ -n "$SAMPLE_H5" ]; then
...
# =============================================================================
echo "=========================================="
echo "PREPROCESSING VV JOB SUMMARY"
echo "=========================================="
echo "Version: VV (Brink's validated methodology)"
echo "Filtering: Per-night temperature (27°C)"
echo "TST range: 3-12 hours"
echo "Max nonwear: 2 hours per night"
echo "Job ID: $LSB_JOBID"
...
if [ $PREPROCESSING_EXIT_CODE -eq 0 ] && [ $TOTAL_OUTPUT -gt 0 ]; then
    echo "✓ PREPROCESSING VV COMPLETED SUCCESSFULLY"
    echo "Ready for feature extraction stage"
    echo "Next: Run feature_extraction with prep_vv data"
    exit 0
else
    echo "✗ PREPROCESSING VV FAILED"
