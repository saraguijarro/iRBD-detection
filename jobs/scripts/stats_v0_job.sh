...
#BSUB -R "span[hosts=1]"
#BSUB -W 6:00
#BSUB -o /work3/s184484/iRBD-detection/jobs/logs/stats/stats_v0_%J.out
#BSUB -e /work3/s184484/iRBD-detection/jobs/logs/stats/stats_v0_%J.err
#BSUB -N

# STATISTICAL ANALYSIS JOB SCRIPT FOR iRBD DETECTION PIPELINE - VERSION V0
...
mkdir -p results/statistical_analysis_v0
...
echo "Executing: python scripts/statistical_analysis.py --version v0"
python scripts/statistical_analysis.py --version v0
...
# Check for output files in results/statistical_analysis_v0
TOTAL_OUTPUT_FILES=0

if [ -d "results/statistical_analysis_v0" ]; then
    STATS_COUNT=$(find results/statistical_analysis_v0 -type f 2>/dev/null | wc -l)
    echo "Statistical results files: $STATS_COUNT"
    TOTAL_OUTPUT_FILES=$((TOTAL_OUTPUT_FILES + STATS_COUNT))
fi

if [ -d "results/visualizations/v0" ]; then
    VIZ_COUNT=$(find results/visualizations/v0 -type f 2>/dev/null | wc -l)
